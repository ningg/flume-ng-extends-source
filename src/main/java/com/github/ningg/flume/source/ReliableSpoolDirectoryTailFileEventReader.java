...
public class ReliableSpoolDirectoryTailFileEventReader implements ReliableEventReader{
	...
	public List<Event> readEvents(int numEvents) throws IOException {
		...
		System.out.println("file is "+currentFile.get().getFile().getName());
	    EventDeserializer des = currentFile.get().getDeserializer();
	    List<Event> events = des.readEvents(numEvents);
	    System.out.println("first message is "+new String(events.get(0).getBody(),"UTF-8"));
	    System.out.println("last message is" +new String(events.get(events.size()-1).getBody(),"UTF-8"));
